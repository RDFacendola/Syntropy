...
namespace Syntropy
{
    ...
    inline String ConsoleLineBuilder::NewLine(const StringView& fill) const
    {
        ...
        auto destination_span = Memory::RWByteSpan(Memory::ToRWBytePtr(line.data()), ToInt(line.size()));
        auto source_span = Memory::ByteSpan(Memory::ToBytePtr(fill.data()), ToInt(fill.size()));
    ...
    {
        auto destination_span = Memory::RWByteSpan(Memory::ToRWBytePtr(destination.data()), ToInt(destination.size()));
        auto source_span = Memory::ByteSpan(Memory::ToBytePtr(source.data()), ToInt(source.size()));
