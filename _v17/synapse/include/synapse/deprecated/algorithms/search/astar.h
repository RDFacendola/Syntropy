...
namespace Syntropy::synapse
{
    ...
    Vector<const TNode*> AStar(const TNode& start, const TNode& end, TAdjacencyFunc adjacency_func, TCostFunc cost_func, THeuristicFunc heuristic_func)
    {
        ...
        std::priority_queue<TFrontierNode, Vector<TFrontierNode>, FrontierNodeComparator> frontier;    // Nodes to be explored yet.
        Map<const TNode*, TFrontierNode> node_map;                                           // Associate each node with the predecessor having the lowest cost and the cost needed to reach it from start.
