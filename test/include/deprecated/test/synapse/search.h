...
private:
    ...
    struct GraphNode
    {
        ...
        const syntropy::Vector<GraphNode*>& GetNeighbors() const;
    ...
    private:
        ...
        syntropy::Vector<GraphNode*> neighbors_;                     ///< \brief List of this node's neighbors.
        syntropy::Vector<float> costs_;                              ///< \brief Cost to get to each neighbor node. Must have the same size as neighbors_.
    };
    ...
    private:
        ...
        syntropy::Vector<std::unique_ptr<GraphNode>> nodes_;     ///< \brief Nodes in the graph.
    };

    /// \brief Create a path on a graph.
    template <typename... TNode>
    syntropy::Vector<const GraphNode*> MakePath(TNode&... ts)
    {
        return { &ts... };
    }
